...
Дополнительные опции в службу можно передать так:

Во внешнем файле `node_exporter`, который мы указываем в `EnvironmentFile=` задаём переменную, например, `OPTIONS="--log.level=info"`

Далее, в `ExecStart=` указываем основную команду с этой переменной из файла`ExecStart=/usr/local/bin/node_exporter $OPTIONS`.
По факту, строка запуска будет такой - `/usr/local/bin/node_exporter --log.level=info`

Во внешнем файле можно указать несколько переменных для передачи опций, а можно все в одной переменной.
![](image/node_exporter2.png)
